...
- [ ] 是否采纳规范代号 **DXEP** 与路径前缀 `/api/dx/v1/`？  
- [ ] 是否同意 **不直接对外暴露** Drupal JSON:API 原始形状？  
- [ ] Channel 匿名读公开内容：允许 / 仅 token？  
- [ ] 批次包部分失败策略：默认继续（非原子）是否可接受？  
- [ ] `notice` 与 `article` 分类型，或先统一为 `article` + 扩展文号？  
- [ ] 金额字段用字符串：是否确认？  
- [ ] 与现有 `dx_xmt_bridge`：过渡双写，还是一次性切 DXEP？  
- [ ] 确认前是否冻结接口实现（仅文档 + 可选 OpenAPI 草稿）？
