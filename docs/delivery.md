...
| 路径 | 说明 |
|------|------|
| `/`（www 营销首页） | 交钥匙叙事 + CTA → `/deliver`（D7-A） |
| `/deliver` | 交付台首页 |
| `/deliver/wizard` | 页面选型下单 |
| `/deliver/chat` | 对话下单 |
| `/deliver/blueprint/{id}` | 蓝图确认 / 验收（含 L3 `manual_todos`） |
...
5. **移植**：L1/L2 调 `dx_migrate` → DXEP Ingest（L2 跟详情；无 URL 时用 fixture）；**L3** 写入验收 `manual_todos`（范围/源站/工单），不假装一键  
6. 验收报告 JSON 存蓝图实体（含 `trust_policy` / `capabilities` / `migrate` / `manual_todos`）  

蓝图页 `/deliver/blueprint/{id}` 以步骤清单展示验收；L3 另列「人工待办」。

## 冒烟

```bash
./scripts/ci/delivery-smoke.sh
./scripts/ci/www-deliver-smoke.sh
