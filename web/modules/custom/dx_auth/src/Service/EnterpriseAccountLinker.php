...
class EnterpriseAccountLinker {
  ...
  /**
   * Credit IDs bound to one Drupal user.
   *
   * @return array<int, array{credit_code: string, company_name: string}>
   */
  public function creditCodesForUid(int $uid): array {
    if ($uid <= 0) {
      return [];
    }
    try {
      $rows = $this->database->select('dx_auth_enterprise', 'e')
        ->fields('e', ['credit_code', 'company_name'])
        ->condition('uid', $uid)
        ->orderBy('id', 'DESC')
        ->execute()
