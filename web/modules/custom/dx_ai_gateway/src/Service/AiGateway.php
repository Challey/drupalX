...
class AiGateway {
  ...
  /**
   * Returns the default provider machine name, including tenant override.
   */
  ...
  /**
   * Builds tenant and published product knowledge for the system prompt.
   */
  public function getKnowledgeContext(): string {
    $tenant = $this->configFactory->get('dx_tenant.settings');
    $parts = [];
    $companyName = trim((string) ($tenant->get('company_name') ?: ''));
    $industry = trim((string) ($tenant->get('industry') ?: ''));
    $customKnowledge = trim((string) ($tenant->get('ai_knowledge_intro') ?: ''));
    if ($companyName !== '') {
    ...
    if ($this->entityTypeManager !== NULL) {
      try {
        ...
        foreach ($storage->loadMultiple($ids) as $node) {
          ...
          $description = $node->hasField('body')
          ...
          if ($description !== '') {
            $line .= ' 描述: ' . mb_substr($description, 0, 160);
          }
          $products[] = $line;
        }
        if ($products !== []) {
        ...
        $this->logger->notice('AI knowledge product loading skipped: @message', [
          '@message' => $exception->getMessage(),
        ]);
      }
    }
  ...
  /**
   * Builds the effective platform/tenant prompt with knowledge context.
   */
  protected function getEffectiveSystemPrompt(): string {
    $tenantPrompt = trim((string) ($this->configFactory->get('dx_tenant.settings')->get('ai_system_prompt') ?: ''));
    $platformPrompt = trim((string) ($this->configFactory->get('dx_ai_gateway.settings')->get('system_prompt') ?: ''));
    $parts = [];
    if ($tenantPrompt !== '') {
      $parts[] = $tenantPrompt;
    }
    elseif ($platformPrompt !== '') {
      $parts[] = $platformPrompt;
    }
