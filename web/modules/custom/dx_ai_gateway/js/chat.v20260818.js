(function (Drupal, once, drupalSettings) {
      ...
      once('dx-ai-chat', '.dx-ai-chat', context).forEach((widget) => {
        ...
        form.addEventListener('submit', async (event) => {
          ...
          try {
            ...
            history.pop();
            const row = pending.closest('.dx-ai-chat__row');
            (row || pending).remove();
  ...
  function appendMessage(container, role, content, pending) {
    ...
    row.className = `dx-ai-chat__row dx-ai-chat__row--${role}`;
    const bubble = document.createElement('div');
    bubble.className = `dx-ai-chat__message dx-ai-chat__message--${role}` + (pending ? ' dx-ai-chat__message--pending' : '');
