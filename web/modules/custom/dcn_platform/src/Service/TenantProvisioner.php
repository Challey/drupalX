...
namespace Drupal\dcn_platform\Service;
...
class TenantProvisioner {
  ...
  public function provision(Tenant $tenant): void {
    ...
    try {
      ...
      $suffix = getenv('DCN_TENANT_SUFFIX') ?: $this->configFactory->get('dcn_platform.settings')->get('default_tenant_suffix') ?: 'drupalx.local';
  ...
  protected function createDatabase(string $machineName): string {
    $prefix = getenv('DCN_DB_PREFIX') ?: 'dcn_tenant_';
    $databaseName = $prefix . preg_replace('/[^a-z0-9_]/', '_', strtolower($machineName));

    $host = getenv('DCN_DB_HOST') ?: '127.0.0.1';
    $port = getenv('DCN_DB_PORT') ?: '3306';
    $user = getenv('DCN_DB_USER') ?: 'root';
    $pass = getenv('DCN_DB_PASS') ?: '';
...
PHP;
  ...
  protected function runSiteInstall(Tenant $tenant, string $machineName, string $databaseName): void {
    ...
    $user = getenv('DCN_DB_USER') ?: 'root';
    $pass = getenv('DCN_DB_PASS') ?: '';
    $adminUser = getenv('DCN_ADMIN_USER') ?: 'admin';
    $adminPass = getenv('DCN_ADMIN_PASS') ?: 'admin';
    $adminMail = $tenant->get('owner_mail')->value ?: (getenv('DCN_ADMIN_MAIL') ?: 'admin@drupalx.local');
    ...
    $timeout = (int) ($this->configFactory->get('dcn_platform.settings')->get('provision_timeout') ?: 600);
