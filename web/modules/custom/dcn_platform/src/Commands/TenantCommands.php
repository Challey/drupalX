...
namespace Drupal\dcn_platform\Commands;
...
class TenantCommands extends DrushCommands {
   ...
   *   Owner email address.
   *
   * @command dcn:tenant-provision
   * @aliases dcn-tp
   * @usage drush dcn:tenant-provision acme --label="Acme Corp" --mail=owner@acme.com
   */
  public function tenantProvision(string $machine_name, array $options = ['label' => NULL, 'mail' => NULL]): void {
    ...
    if ($existing) {
      ...
      $tenant = Tenant::create([
        ...
        'label' => $label,
        'owner_mail' => $options['mail'] ?: (getenv('DCN_ADMIN_MAIL') ?: 'admin@drupalx.local'),
        'status' => 'draft',
      ]);
      $tenant->save();
